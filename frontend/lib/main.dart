...
const Map<AppLanguage, Map<String, String>> _strings = {
  AppLanguage.en: {
    ...
    "selectedPdf": "Selected PDF: {name} • {category}",
  ...
  AppLanguage.am: {
    ...
    "selectedPdf": "የተመረጠ PDF: {name} • {category}",
...
class _PostsHomePageState extends State<PostsHomePage> {
  ...
  String _tf(String key, Map<String, String> values) {
    var template = _t(key);
  ...
  Future<void> _saveLyric() async {
    ...
    try {
      final uri = _editingId == null
          ? Uri.parse("$apiBaseUrl/api/posts")
          : Uri.parse("$apiBaseUrl/api/posts/$_editingId");
  ...
  Future<void> _deleteLyric(String id) async {
    try {
      final response =
          await http.delete(Uri.parse("$apiBaseUrl/api/posts/$id"));
      if (response.statusCode >= 400) {
  ...
  Future<void> _fetchCategories() async {
    ...
    try {
      final response = await http.get(Uri.parse("$apiBaseUrl/api/categories"));
      if (response.statusCode >= 400) {
  ...
  Future<void> _fetchSubcategories({String? categoryId}) async {
    ...
    try {
      final uri = categoryId == null || categoryId.isEmpty
          ? Uri.parse("$apiBaseUrl/api/subcategories")
          : Uri.parse("$apiBaseUrl/api/subcategories?categoryId=$categoryId");
  ...
  Future<void> _updateSubcategory({
    ...
    try {
      final response = await http.put(
        Uri.parse("$apiBaseUrl/api/subcategories/$id"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({"name": name, "categoryId": categoryId}),
      );
      if (response.statusCode >= 400) {
  ...
  Future<void> _deleteSubcategory({
    ...
    try {
      final response =
          await http.delete(Uri.parse("$apiBaseUrl/api/subcategories/$id"));
      if (response.statusCode >= 400) {
  ...
  Future<void> _deleteCategory(String id) async {
    try {
      final response =
          await http.delete(Uri.parse("$apiBaseUrl/api/categories/$id"));
      if (response.statusCode >= 400) {
  ...
  Future<void> _updateCategory(String id, String name) async {
    try {
      final response = await http.put(
        Uri.parse("$apiBaseUrl/api/categories/$id"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({"name": name}),
      );
      if (response.statusCode >= 400) {
  ...
  Future<void> _importPdf() async {
    ...
    input.onChange.first.then((_) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _tf("selectedPdf", {"name": file.name, "category": category}),
          ),
        ),
      );
    });
